...
- **Pins persist to disk.** This is a deliberate amendment to the privacy invariant: *unchosen* conversation content still never touches disk (the activity log stays in-memory), but content the user explicitly pinned lives in `pins.json` and is deleted with the pin. In-memory-only and safeStorage-encrypted variants were rejected — the first makes the todo list vanish on quit, the second adds keychain prompts on every ad-hoc rebuild. **Superseded 2026-09-23** (`2026-09-23-pins-at-rest-design.md`): the file is now a `safeStorage`-sealed envelope. The prompt objection lapsed once `passkeys.json`, `lock.json` and cookie encryption all opened the same keychain item at every boot; a legacy plaintext file is re-sealed at first launch.
