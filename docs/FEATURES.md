...
- **Settings → Diagnostics** — a bounded in-memory ring (`DIAG_CAP` = 200) of the evidence lines main already printed (`[nav] contained` / `popup denied` / `popup contained`, `[open] miss`, `[identity]`, `[passkey]`, `[ipc]`, `[notifications]`, `[downloads]`) plus runtime transitions it never logged before: `[recipe] <id> stale` / `recovered` (on the flag's change, never per tick), `[view] <id> crashed (attempt n/5, reload in Xs)` / `crashed (cap reached; manual Retry)` / `load failed` / `recovered`, `[peek] <id> peek started` / `peek ended: report|timeout`. Every note is composed main-side from ids and states — `[nav]` URLs are stored as origin + path (`redactUrl`), and no title, body, filename or pin text ever enters the ring. Newest first in the pane, oldest first under a header (Goetia and Electron versions, platform/arch, enabled services, a settings summary) in the text **Copy report** puts on the clipboard. In memory only; both channels shell-only and so refused while locked. Impl: `src/main/lib/diagnostics.ts`, `ctx.diag`, `ViewHooks.note`, `renderer/src/components/DiagnosticsPane.tsx`. Verified: `diagnostics.test.ts`, `resilience.test.ts`, `activate.test.ts`, `ipc-sender-policy.test.ts`, e2e `diagnostics.spec.ts`.
