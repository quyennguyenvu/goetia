...
- **Pinned messages** — right-click selected text in any service → Pin Message, or `Edit ▸ Pin Selection` (⌘/Ctrl ⇧ S — left hand, works inside every page, see Shell shortcuts) where the site owns right-click; Home's Pinned band orders them (pin 0 = in progress, drag to reprioritize), Done/unpin with Undo, click opens the conversation in-page. Each pin names its conversation: from the page title where that carries it, from recipe hooks where it does not (WhatsApp's and Zalo's chat header; Messenger/Instagram's sidebar row for the document URL); WhatsApp pins open by clicking the named chat-list row, Zalo pins by a trusted click main lands on the row (`service:trusted-click` — Zalo ignores synthetic events). Persisted to `pins.json`. Impl: `src/main/pins.ts`, `lib/pin-rules.ts`, `lib/context-menu.ts`, `preload/lib/conversation-open.ts`, `preload/recipes/meta-unread.ts` (`conversationFromRows`), `preload/recipes/whatsapp.ts`, `renderer/src/components/welcome/PinnedBand.tsx`. Verified: `pin-rules.test.ts`, `pins.test.ts`, `context-menu.test.ts`, `state.test.ts`, `conversation-open.test.ts`, `whatsapp-conversation.test.ts` (fixture `whatsapp-chat.html`), `zalo-conversation.test.ts` (fixture `zalo-chat.html`), `meta-conversation.test.ts`, e2e `pins.spec.ts` (incl. a paragraph-length pin: zero horizontal overflow before, during and after a drag).
