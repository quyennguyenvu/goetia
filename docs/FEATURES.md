...
- **Quick switcher** (`⌘/Ctrl+K`) with fuzzy search. Impl: `QuickSwitcher.tsx`, `components/fuzzy.ts`. Verified: `tests/unit/fuzzy.test.ts`.
...
- **Persisted settings** — order, muted, disabled, neverHibernate, theme, railPosition, closeToTray, launchAtLogin, globalMuted. Impl: `src/main/settings.ts`. Verified: `settings.test.ts`.
