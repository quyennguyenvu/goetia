...
- Tile reorder is drag-local: `Reorder.Group` fires `onReorder` on every crossing, so `useTileReorder` holds a draft order in the renderer and sends `service:reorder` once, on `onDragEnd`. Never wire `onReorder` straight to IPC — one drag would be one settings write, one broadcast and one app-menu rebuild per crossing. The draft clears when the broadcast lands, not on commit, so the drop never snaps back for a frame.
